...
export function ManualOrderForm({ open, onClose, onOrderCreated }) {
  const { user } = useAuth();
  ...
  const handleSubmit = () => {
    ...
    // Auto-set payment status to 'partial' if advance > 0 and it's not already 'paid'
    let finalPaymentStatus = paymentStatus;
    if (advancePayment > 0 && finalPaymentStatus !== 'paid') {
        finalPaymentStatus = 'partial';
    } else if (advancePayment >= total) {
        // If advance is equal or more than total, treat it as paid, or stick to explicit paid choice
        finalPaymentStatus = finalPaymentStatus === 'paid' ? 'paid' : 'paid';
    } else if (advancePayment === 0 && finalPaymentStatus === 'partial') {
        // Prevent setting partial if amount is zero
    ...
    resetForm();
    onOrderCreated(); // Refresh the list in OrdersRecord
    onClose();
  };
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 py-4">
          ...
          <div className="space-y-4">
            <h4 className="text-foreground">Customer Details</h4>
            <div>
            ...
            <h4 className="text-foreground pt-4">Order Status & Payment</h4>
            <div>
          ...
          {/* Items */}
          <div className="space-y-4">
            <h4 className="text-foreground">Order Items</h4>
